 ...
 const handleDelete = async (id) => {
  ...
  try {
    ...
    if (data.deletedCount > 0 || data.success) {
      ...
      modelData((prev) => prev.filter((item) => item._id !== id));
    } else {
  ...
  useEffect(() => {
    ...
    const tl = gsap.timeline({
      ...
      delay: 0.2, // route change এর পর 0.2s wait
    });
       ...
       >
         <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 bg-[#11190c] backdrop-blur-md border border-green-800 rounded-2xl shadow-2xl overflow-hidden p-6">
           ...
           <div className="flex flex-col justify-center text-[#4af201] p-4">
             ...
             <div ref={btnRef} className="mt-8 flex gap-4">
           {user ? (
               <button onClick={handlePurchase}className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-300 hover:scale-105">
                 Purchase
               </button>
           ) : (
<h1>Need to login</h1>
